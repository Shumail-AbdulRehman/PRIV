...
export default function LocationsPage() {
  ...
  const totalStaff = locations.reduce(
    (sum: number, loc: LocationWithCounts) => sum + loc._count.staff,
    0,
  );
  const totalTemplates = locations.reduce(
    (sum: number, loc: LocationWithCounts) => sum + loc._count.taskTemplates,
    0,
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Locations"
        subtitle="Organize your sites, assign teams, and manage cleaning schedules."
        action={
          isAdmin ? (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Detected automatically from the latitude/longitude. Only
                      change it if the detected zone is wrong.
                    </p>
                  </div>
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Auto-fill latitude and longitude from this device,
                        ...
                        {capturedLocation.accuracy !== null &&
                          capturedLocation.accuracy >
                            ...
                            <p className="mt-2 text-xs font-medium text-amber-600">
                              Accuracy is low (±
                              {Math.round(capturedLocation.accuracy)} m). For
                              best results, use a phone/tablet with GPS
                              outdoors, or verify the pin on a map before
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <StatCard label="Locations" value={locations.length} icon={MapPin} />
        <StatCard
        ...
        <EmptyState
          ...
          message={
            isAdmin
              ...
              : "No locations assigned to you yet. Contact your admin."
          }
          action={
            isAdmin ? (
              <Button
