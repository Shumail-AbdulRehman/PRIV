...
export default function ChoosePlanPage() {
  ...
  if (subscription.data && !needsPlanSelection(subscription.data.company)) {
    return (
      <Navigate
        to={
          subscription.data.company.subscriptionStatus === "ACTIVE"
            ? "/dashboard"
            : "/subscription"
        }
        replace
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {logout.isError ? (
        <p role="alert" className="mt-4 text-sm text-status-missed">
      ...
      {subscription.isError ? (
        <div role="alert" className="mt-8 space-y-3">
          <p>
            We couldn’t load your workspace. Please try again before choosing a
            plan.
          </p>
          <Button
        ...
        <div className="mt-8">
          <PaddlePricing appearance="marketing" />
        </div>
      )}
      <p className="mt-6 text-sm text-muted-foreground">
        Already finished checkout?{" "}
        <Link to="/welcome" className="font-medium text-primary underline">
