    ...
    try {
      ...
      setServerError(
        apiError.response?.data?.message ??
          "Could not save the manager. Please try again.",
      );
    }
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
          ...
          <fieldset>
            ...
            <p className="mb-2 text-xs text-muted-foreground">
              The manager can only see and manage the locations you select here.
              A location can only be assigned to one manager.
            </p>
            <div className="max-h-48 space-y-1 overflow-y-auto rounded-lg border border-border/70 bg-muted/30 p-2">
              {locations.map((loc) => {
                  ...
                  >
                    <input
                      ...
                      {...register("locationIds", {
                        validate: (value) =>
                          value.length >= 1 || "Assign at least one location",
                      })}
